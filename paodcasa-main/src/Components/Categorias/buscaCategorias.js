...
const ProdutosPorCategoria = ({ route }) => {
  ...
  const handlePesquisa = async (text) => {
    ...
    try {
      const response = await fetch(
        `http://192.168.0.107:3000/api/produto/${text}`
      );
      if (!response.ok) {
  ...
  useEffect(() => {
    fetch(`http://192.168.0.107:3000/api/categoria/${categoriaId}`)
    ...
    fetch(`http://192.168.0.107:3000/api/produto/categoria/${categoriaId}`)
      .then((response) => response.json())
    ...
    <View style={styles.produtoItem}>
      ...
      >
        <View style={styles.itemContainer}>
          <Image
            source={{ uri: `http://192.168.0.107:3000${item?.imagem[0].url}` }}
